...
CORS(
    ...
    origins=["http://localhost:5173", "https://lofi-app-dc75.onrender.com", "https://ai-spotify-app.vercel.app"],
)
...
def generate_song():
    ...
    return jsonify({"message": "Generation started", "task_id": task_id, "status": new_task.status}), 200
...
def callback():
    ...
    if data.get("code") == 200:
        ...
        s3.upload_fileobj(BytesIO(response.content), AWS_BUCKET_NAME, f"{file_name}", ExtraArgs={"ContentType": "audio/mpeg"})
